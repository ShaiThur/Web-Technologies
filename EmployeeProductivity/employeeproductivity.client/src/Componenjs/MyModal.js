...
var _Post = require("../Entites/Post.interface");
...
var MyModal = function MyModal(_ref) {
  ...
  var AddPost = function AddPost() {
    if (title.current.value != "" && date.current.value != "" && difficult.current.value != "" && difficult.current.value <= 3 && difficult.current.value > 0 && description.current.value != "") {
      ...
      console.log(newPost);
      AddPost_PostRequest().then(function (status) {
        var newPostInList = {
          title: newPost.title.toString(),
          deadLine: newPost.deadline.toString(),
          countStars: newPost.complexity.toString()
        };
        setNewPost([].concat(_toConsumableArray(posts), [newPostInList]));
        console.log(status);
  ...
  var AddPost_PostRequest = /*#__PURE__*/function () {
    ...
    return function AddPost_PostRequest() {
